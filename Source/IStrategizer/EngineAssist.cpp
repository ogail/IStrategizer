...
//------------------------------------------------------------------------------------------------------------------------------------------------
EngineAssist& EngineAssist::Instance(RtsGame* p_rtsGameInstance)
{
    static EngineAssist m_instance;
    m_instance.rtsGameInstance = p_rtsGameInstance;
    return m_instance;
}
//------------------------------------------------------------------------------------------------------------------------------------------------
int EngineAssist::GetRequiredResources(PlayerType p_playerIndex, TID p_classId, Cost& p_requiredCost)
{
    ...
    {
        const GameType* m_type = rtsGameInstance->GetEntityType((EntityClassType)p_classId);
    ...
    {
        const GameResearch* m_research = rtsGameInstance->GetResearch((ResearchType)p_classId);
...
int EngineAssist::GetRequiredBuildings(PlayerType p_playerIndex, TID p_classId, vector<EntityClassType>& p_requiredBuildings)
{
    ...
    rtsGameInstance->GetPlayer(p_playerIndex)->TechTree()->GetRequirements(p_classId, requiredResearches, requiredBuildings);
...
{
    p_value = rtsGameInstance->GetPlayer(p_playerIndex)->GetEntity(p_entityObjectId)->Attr(p_attributeId);
    return ERR_Success;
}
//------------------------------------------------------------------------------------------------------------------------------------------------
int EngineAssist::GetSourceBuilding(IN PlayerType p_playerIndex, TID p_entityClassId, EntityClassType& p_sourceBuildingId)
{
    p_sourceBuildingId =  rtsGameInstance->GetPlayer(p_playerIndex)->TechTree()->SourceEntity(p_entityClassId);
    return ERR_Success;
}
//------------------------------------------------------------------------------------------------------------------------------------------------
int EngineAssist::GetObject(PlayerType p_playerIndex, TID p_entityObjectId)
{
    int m_exists = rtsGameInstance->GetPlayer(p_playerIndex)->GetEntity(p_entityObjectId) != nullptr;
...
{
    int m_exists = rtsGameInstance->GetPlayer(p_playerIndex)->TechTree()->ResearchDone(p_researchId);
    return ((m_exists) ? ERR_Success : ERR_EntityDoesNotExist);
}
//------------------------------------------------------------------------------------------------------------------------------------------------
int EngineAssist::GetResourceAmount(PlayerType p_playerIndex, ResourceType p_resourceId, int& p_availableAmount)
{
    PlayerResources* m_resources = rtsGameInstance->GetPlayer(p_playerIndex)->Resources();
...
{
    GameEntity* m_entity = rtsGameInstance->Self()->GetEntity(p_sourceBuildingObjectId);
...
{
    GameEntity* m_entity = rtsGameInstance->Self()->GetEntity(p_workerObjectId);
...
{
    GameEntity* m_entity = rtsGameInstance->Self()->GetEntity(p_trainerObjectId);
...
{
    GameEntity* m_entity = rtsGameInstance->Self()->GetEntity(p_entityObjectId);
...
{
    GameEntity* m_entity = rtsGameInstance->Self()->GetEntity(p_attackerObjectId);
...
{
    rtsGameInstance->Players(p_playerIds);
    return ERR_Success;
}
//------------------------------------------------------------------------------------------------------------------------------------------------
...
int EngineAssist::GetPlayerColonies(IN PlayerType p_playerType, OUT vector<Colony>& p_colonies)
{
    ...
    GameEntity* m_entity;
    GamePlayer* m_player = rtsGameInstance->GetPlayer(p_playerType);
    int m_posX, m_posY;
...
{
    //FIXME: provide a valid implementation
    assert(0);
   /* vector<TID> m_entities;
    ...
    case PATTRIBUTE_EntitiesCount:
        rtsGameInstance->GetPlayer(p_playerType)->Entities(m_entities);
...
int EngineAssist::GetEntityObjectFromEntityClass(PlayerType p_playerIndex, EntityClassType p_classId, const vector<TID>& p_execludedIds, TID& p_objectId)
{
    ...
    GamePlayer* m_player = rtsGameInstance->GetPlayer(p_playerIndex);
...
{
    p_entityClassId = rtsGameInstance->GetPlayer(p_playerType)->TechTree()->TireBaseBuilding(p_baseType);
    return ERR_Success;
}
//----------------------------------------------------------------------------------------------
...
int EngineAssist::GetFilterCount(PlayerType p_playerIndex, FilterType p_filterIndex, IN const Vector2& p_cellIndex, int& p_count)
{
    ...
    else
    {
        ...
        {
            type = rtsGameInstance->GetEntityType(m_player->GetEntity(allEntities[i])->Type());
...
int EngineAssist::GetFilterCount(PlayerType p_playerType, FilterType p_filterIndex, IN const Colony& p_colony, int& p_count)
{
    ...
    m_player = rtsGameInstance->GetPlayer(p_playerType);
    ...
    for(int i = 0, size = m_entities.size(); i < size; ++i)
    {
        ...
        m_entityType = rtsGameInstance->GetEntityType(m_entity->Type());
...
bool EngineAssist::DoesEntityClassExist(pair<EntityClassType, unsigned> p_entityType, PlayerType p_playerType)
{
    ...
    pPlayer = rtsGameInstance->GetPlayer(p_playerType);
    assert(pPlayer);
...
bool EngineAssist::DoesEntityClassExist(const map<EntityClassType, unsigned> &p_entityTypes, PlayerType p_playerType)
{
        ...
        itr != p_entityTypes.end(); ++itr)
    {
        ...
        for (size_t i = 0, size = entities.size(); i < size; ++i)
        {
            ...
            {
                pType = rtsGameInstance->GetEntityType(itr->first);
                assert(pType);
...
int EngineAssist::PrerequisitesSatisfied(int p_entityOrResearchType, bool &p_satisfied, PlayerType p_playerType)
{
    ...
    p_satisfied = prerequisitesExpression->Evaluate(rtsGameInstance);
...
{
    GameEntity* entity = rtsGameInstance->Self()->GetEntity(p_entityId);
    assert(entity);
...
void EngineAssist::GetPrerequisites(int p_entityOrResearchType, PlayerType p_playerType, vector<Expression*>& p_prerequisites)
{
    ...
    {
        pResearchType = rtsGameInstance->GetResearch((ResearchType)p_entityOrResearchType);
    ...
    {
        pEntityType = rtsGameInstance->GetEntityType((EntityClassType)p_entityOrResearchType);
        assert(pEntityType);
